...
§6.2 原写法是"井宽以世界单位固定、井数 ∝ 面积、RNG 顺序不变"。在动手实现前先量了这个构造的
**场统计量是否随 L 不变**（同一批 seeds，网格 ∝ L 使 dx 保持 1.5625）：

| 构造 | L=100 (64²) | L=200 (128²) | L=400 (256²) |
|---|---|---|---|
| mixture，绝对井宽 + 井数 ∝ 面积：对比度 std/mean | 0.9375 | 1.2135 (**+29%**) | 1.3088 (**+40%**) |
| 同上：相关长度（世界单位） | 16.67 | 21.09 (**+27%**) | 24.74 (**+48%**) |
| CRF，`correlation_fraction = ℓ/L`：对比度 | 0.4028 | 0.3123 (**−22%**) | 0.2799 (**−31%**) |
| 同上：相关长度（世界单位） | 13.80 | 13.80 (**+0%**) | 14.06 (**+2%**) |

两条结论：

1. **相关长度可以靠一个参数做到不变**：`correlation_fraction = ℓ/L`（把该族的参数化从"格"改成
   "世界单位"）即可，实测 13.80 → 13.80 → 14.06。
2. **对比度在两个族里都做不到不变**：`field -= field.min(); field /= field.mean()` 会在域变大时
   见到更多极值，从而改变归一化后的形状。mixture 上 +40%，CRF 上 −31%。

所以"井宽固定 + 井数 ∝ 面积"**不是**一个纯粹的尺寸操纵——它同时把场的对比度改了 40%，
而对比度直接决定效应量级。按本设计自己定的 conflation 标准（§4.3 P4），这个构造不能直接用。
把对比度钉住只需在归一化后加一行重标定，于是剩下三个候选（见 §6.2.1）。

**同时保留的兼容性要求**：参考域 L=100 时井数 = 5、井宽 ∈ 6–18 世界单位，
RNG 抽取顺序不变，因此 L=100 的参数与现行逐位相同（采样格不同的问题见 §6.3）。

#### 6.2.1 Leg S 的景观构造：三个候选

| # | 构造 | 场统计量 | 族连续性 | 代价 |
|---|------|---------|---------|------|
| S1 | **复制参考尺度补片**：L 倍的域由 k 个独立的参考尺度补片拼成，每片用自己的 seed | 每片的场与粒子数**按构造**与参考系统同分布（k 片 × N/k = N_ref 粒子） | ✔ 仍是 `gaussian_mixture` | 域内没有长波景观异质性（须写进"不声称"）；且每片只有参考级的域/相关长度比，测不到该比值的变化 |
| S2 | **单个更大场，用 CRF + 世界单位相关长度 + 对比度钉住** | 相关长度 ✔（实测不变）、对比度由重标定钉住 ✔ | ✘ 换到保留族，Leg S 变成 Leg H 的条件子句 | 需要 CRF 的参数化改造 |
| S3 | **单个更大场，mixture + 绝对井宽** | 对比度 +40%、相关长度 +48%，两者都须作为"被操纵量"声明 | ✔ | leg 不再是纯尺寸改变：效应若变，无法归因 |

配套的测量方式都成立：Leg S 的估计量取自**瓦片化的参考尺度窗口**（§4.1），
在 S1 下窗口就是补片本身；在 S2/S3 下是域内平铺的参考尺度窗口。
