...
`k = 1`、`η_d = 0.5`、`η_n = 0.05`；`w_ref = ability_saturation_w` 未在 config 中给出，
取编译期默认值 **5.0**（`core/config.hpp:52`），恰好等于配置的平均财富 `mean_wealth = 5`，
因此 `use_saturation = true`，能力函数在均值财富处处于半饱和点——`w_ref` 事实上是
能力异质性能否有效转化为交换优势的尺度参数，改动它等价于改动模型，不是数值细节。
