...
| 配置面完备性 | 生成的 `politeia.cfg` 显式声明 185 个可解析键中的 47 个 | 其余 138 个取 C++ 类内默认值；逐键审计后 136 个中性（模块开关/IC 文件/未走分支/哨兵/别名/无消费者），2 个在参考配置下可达且非中性而未声明：`density_radius`（决定 cell 几何与串行原地交换的枚举顺序）、`culture_dim`（决定 IC 装载消耗的随机数个数，从而决定初始动量）。见台账 §4.8 与 `research/config-coverage-audit.md` |
...
4. **存储顺序误差界的覆盖缺口（S18）**：V1F 的 `order` 层被强制以 `temperature = 0.0`
   运行，而该层实测的 0 值/非 0 结构（`wealth_gini` 非 0 且随 dt 收缩，位置类指标恰好 0）
   正是在这个温度下取得的；参考配置用 `temperature = 0.5`，此时热噪声按数组行序指派，
   这一耦合从未被任何一层覆盖。需一次廉价实测判定，见台账 §4.7；
