...
日期：2026-09-17（V1G 判定与其记录补于 2026-09-18）。状态：Cycle 4 代码级修复、顺序敏感性和非平坦步长误差界已通过；V1F 64-seed 独立校准已完成并**全部 Gate 通过**（2026-09-17 08:09 +08:00，960/960 runs，0 失败，340.95 CPU 小时 / 46.13 墙钟小时）；promotion `archive-v1f` → `prepare` → V0G → `finalize` 已完成，Cycle 4 最终参数锁已冻结并授权 `E1-MATCHED-LANDSCAPES-C4`。**E1-C4 已完成并归档**：128/128 runs、0 失败、18512 s 墙钟与 34.97 CPU 小时、六道分析 Gate 全通过，三个必需空间指标上的 `clustered − shuffled` 配对效应均超过冻结 SESOI，故 `C2-LANDSCAPE-C4` 判为 `supported`。**V1G 诊断为 `bounded`**（2026-09-17，128 runs，见 §4.7），故 **S18 关闭为"已量化"**——冻结的数值上限覆盖参考温度下的存储顺序通道；两处保留随结论引用：`occupancy_entropy` 已用掉冻结上限的 88%，且 12/128 run 未通过逐 run 稳态窗口检查（按预注册不参与判定）。
...
| S18 | P1 | A/C | V1F 的 `order` 层以 `temperature = 0.0` 运行（`run_v1_calibration.py:69-75` 强制），因此**存储顺序误差界从未在有热噪声（`temperature = 0.5`，即 E1-C4/E2-C4 的实际配置）时被检验**：热噪声按数组下标消费，而 `redistribute`/`migrate_particles` 在 `nprocs=1` 时直接返回，故参考配置下数组顺序恒等于 IC 文件行序，换行序即换噪声指派。该层实测：`wealth_gini` 2.20e-4→9.38e-5（随 dt 收缩，上限 0.01，pass），其余三指标恰好 0.0——这个结构自洽并**支持** R08（参考配置里唯一的成对原地操作是交换，只改财富；位置类指标因 `social_strength=0` 只剩单粒子地形力而与行序无关） | 审计定位；**实测已判定**（T=0.5 下 canonical vs permuted 的尾部指标差） | **已关闭**（2026-09-17）：实测 `bounded`——四项界 0.001845 / 0.001503 / 0.000876 / 0.001086 全部 ≤ 冻结上限 0.004590 / 0.003259 / 0.000995 / 0.001441，故该界**覆盖**热噪声↔行序耦合。两处保留：`occupancy_entropy` 已占上限 88%（余量 12%），且 12/128 run 未过逐 run 稳态检查（不参与判定）。判定与实测表见 §4.7；记录 `research/jobs/V1G-ORDER-THERMAL-C4/{order_thermal_report,result,manifest}.json`（预注册 `research/v1g-order-thermal-design.md`、脚本 `run_v1g_order_thermal.py`、18 项测试已入库） | **无需修复**：不必把热噪声改为按稳定 GID 抽取，无需 C++ 改动，无需重做校准与授权链 |
...
**判定已完成：`bounded`（2026-09-17，jobctl pid 2149600，128 runs，墙钟 13717 s ≈ 3.81 h）**：
`V1G-ORDER-THERMAL-C4` 就是 V1F 的 order 层只改一个参数（`T 0.0 → 0.5`），景观/dt/时长/
尾窗口/64 个 seed 全部不动，IC 用显式相态以保证两份输入是同一物理态、只有行序不同。
判定规则预先冻结：每指标按 seed 配对 `|mean| + 2·SE ≤ 已冻结 numerical_resolution_limits`，
阈值从参数锁读出并重算校准文件 SHA，且只允许跑冻结的参考 binary（S14 检查）。
预注册见 `research/v1g-order-thermal-design.md`。

实测（`research/jobs/V1G-ORDER-THERMAL-C4/order_thermal_report.json`，sha256 `93a01493…`；
记录见同目录 `result.json` 与 `manifest.json`，由 `record-diagnostic` 机械导出而非手抄）：

| 指标 | T=0.5 的 `|mean|+2·SE` | 冻结上限 | 占上限 |
|---|---|---|---|
| `resource_density_spearman_rho` | 0.001845 | 0.004590 | 40% |
| `density_morans_i` | 0.001503 | 0.003259 | 46% |
| `occupancy_entropy` | 0.000876 | 0.000995 | **88%** |
| `wealth_gini` | 0.001086 | 0.001441 | 75% |

四项全部落在冻结上限内，故 **S18 关闭为"已量化"**：在 64 seed、dt=0.005、尾 144 帧这一
层面上，已冻结的数值上限**覆盖**参考温度下的存储顺序通道。因此不需要把热噪声改为按
**稳定 GID** 抽取，也就不需要那项 C++ 改动与随之而来的重做校准与授权链。

**两个必须随结论一起引用的保留**（预注册 §5/§6 已先写死，非事后补）：

1. **`occupancy_entropy` 只剩 12% 余量**（0.000876 / 0.000995 = 88%）。其余三项余量为
   54%–75%。热噪声对该指标的消耗远大于其余三项，所以"覆盖"对它是最勉强的：任何使该项
   噪声增幅超过 **13.6%**（比值 0.000995/0.000876）的改动都会先在此处越界。这不改变本次
   判定，但它是下一次校准最该盯的指标。
2. **12/128 个 run 未通过逐 run 稳态窗口检查**（报告里的 `window_caveat`）。按预注册，
   窗口健康度**不参与判定**（V1G 是两单元设计，无法复现校准用的 3×3 condition-ensemble
   两窗口 Gate），但界继承了这份不稳定性，引用时须带此保留；n=64 也不比 V1F 更强。

两种结果都不改变任何已冻结阈值，也不改变 E1-C4 的任何结论——它的 IC 文件固定，
存储顺序不是它的误差来源。

**共有的结构成因。** S16、S17 与前述 S18 的**关切**同属一种模式：**一个未接入/未验证的
通道没有在入口处被拒绝**，于是它可以在没人注意时被打开。处理方向一致：让入口直接拒绝
（MPI 与 restart 的非显式 opt-in 一律失败），而不是依赖文档警告。

**但 S18 的实测结果与这个模式相反，这一点值得单独记下。** S18 是"**未验证**"而非
"**未拒绝**"：V1G 的判定显示既有的数值上限**已经覆盖**热噪声↔行序耦合，因此它不需要 C++
改动、不需要重跑校准（见 §4.7）。S16/S17 的修复才确实改动 C++，故排在 E1-C4 之后；按
S14 的结论，任何 C++ 改动都会改变参考 binary 的 SHA，必须连带重跑校准。把这两类分开是
有意义的：**"从未验证过"不等于"有问题"**，而把前者当成后者会付出一次本不必要的重校准
代价——S18 正是这个代价的反例。
...
**S18 已关闭（2026-09-17）**：V1G-ORDER-THERMAL-C4 的判定为 `bounded`——参考温度下的
四项存储顺序界全部落在冻结上限内，故原先的"该界只在 `T=0` 标定过"这一残余不再成立。
判定与实测表见 §4.7。**但关闭不等于余量充裕**：`occupancy_entropy` 的界已用到冻结上限
的 88%，是四项中最紧的，任何使该项噪声增幅超过 13.6% 的改动都会先在此处越界——下一次
校准应优先看它，而不是看已经关闭的结论本身。另外 12/128 个 run 未通过逐 run 稳态窗口
检查，按预注册不参与判定，但引用该界时须带此保留。

E1-C4 jobctl 台账中的提交时刻（08:10）与实际不符（run 目录创建于 10:09、结果写于 15:17），
属记账瑕疵，不影响证据链。
...
归属于 **3 个 component**。审计对未登记的跨 job 复用直接报错，而 E2-C4
故意不在 `SEED_REUSE_COMPONENTS` 内，因此它一旦撞用旧 seed 会在提交前失败。
component 的 `status` 字段**纯描述、不参与放行**——放行只看"成员集合是否包含全部消费者"，
有一条测试专门反转标签以锁住这一点。

**第三处欠账：授权靠的是我的叙述，而非可验证的性质（2026-09-17 自查发现）。**
上一版的三条 component 只用自由文本 `reason` 放行。逐条回查已提交证据后：

| component | 我原先写的理由 | 回查结果 |
|---|---|---|
| V1F↔V1G | "配对再运行，seed 配对即估计量" | **有据**：`v1g-order-thermal-design.md:33` 明写 seeds = "V1F 冻结的 64 个"并给出理由 |
| B0 C1–C3 | "跨 C1–C3 固定 seed **是刻意的**" | **"刻意"无据**；可查实的只是三个 job 的 `experiment_id` **完全相同**（`B0-DYNAMICS-PILOT`），即同一实验的三个 cycle 记录，属**重执行产物**而非"借用他人随机流" |
| E0/E1/E2 | "Cycle 3 的 E2 复跑了 E1 的 seed" | **无据**：全仓库**没有任何文档**记载此事。它是**从未被写成决策的碰撞**，正因此才算缺陷 |

即"E2 复跑 E1 seed"是真事实（两份 seed 列表逐项相同），但"复跑"这个**意图**是我编的；
`notes/b0-budget-approval.md` 只列了 B0 的 3 个 seed，从未说跨 cycle 固定。

修法不是把理由写得更漂亮，而是让每条授权带一个**可机器校验的 basis**：

| basis | 机器校验（不满足即报错） | 适用 |
|---|---|---|
| `same_experiment_reexecution` | 全部成员的 `experiment_id` 必须**完全相同** | B0 三连 |
| `declared_paired_rerun` | `citation` 指的文件必须**存在**且**仍含**登记的 `marker` 文本 | V1F↔V1G |
| `historical_collision` | 该 component 的共享 seed 必须与 `EVIDENCE_SEED_SETS` **不相交** | E0/E1/E2 |

第三行才是**真正要紧的那条性质**：这些碰撞之所以无害，不是因为我说它们无害，而是因为
**它们碰不到任何承载精度界、阈值或确认性主张的 job**。实测：grandfathered 的 24 个 seed
（全部 ≤5519）与证据集（8 个 evidence job、174 个 seed、最小 6007）**完全不相交**。
`EVIDENCE_BEARING_JOBS` 里的每个名字也必须存在，否则该不相交断言会**空洞成立**——这也有
测试守着。

**变异检验**：把校验函数临时改成空操作后，**恰好 8 项**校验测试失败（三条 basis 各若干、
未知 basis、成员缺失、证据 job 名拼错），确认检查承重而非装饰。随后已还原。

另一处**已查实**、对将来有用的结构性表述：B0 与 E0 的 `-C2`/`-C3` 目录与其首代共享
`experiment_id`，所以它们是**同一实验的多次记录**，不构成独立样本；将来若有人想把它们
当作重复样本池化，这个 `experiment_id` 是现成的判据。
...
**测试**：`tests/test_prepare_cycle4_confirmation.py` 新增 **36 项**（三条通道各自的抓取、
sentinel 处理、嵌套 `seeds` 判为引用、重分析 job 的重分类与其 `source_experiment` 佐证、
单文件内重复、`seeds.txt` 与 config 不一致、未登记复用报错、`status` 标签不参与放行、
无证据且无 waiver 报错、无证据但有 waiver 通过、非素数不报错、池与提议的边界；**三条 basis
各自的通过与失败路径**，含引文文件缺失、引文文件被改写后不再含声明、缺引文、未知 basis、
成员目录缺失、`experiment_id` 不一致、证据 job 名拼错、以及"把证据配对误标为无害历史碰撞"
这一变异；另有一项对真实仓库跑全量审计的集成守护，其中显式锁住"V1D 不得被记为消费者"
与"grandfathered 集合与证据集不相交"）。全库 **272 项**测试通过。
