...
覆盖**，包括 wealth_gini 那 9.4e-5——因为 `T=0` 时该耦合不存在。判定成本很低、不需要
新的模型代码：umi 上做一次 V0 规模实测，固定 `temperature = 0.5`，同一批 seed 各跑
canonical 与 permuted 两份按 GID 相同的显式相态输入，比较尾部指标差。若仍在数值上限内，
则该界成立（只是当初测的层更窄）；若超出，则参考配置的数值误差界缺一条腿，需要改为
**不依赖行序的噪声指派**（按稳定 GID 取噪声，与 S07 对交换噪声的做法一致）。
