...
`mean_wealth` **不在**保留的 CSV 中（62 列里没有它），故其结果上限须从保留快照重算；
快照完整（864,960 个），所以仍是重分析而非重跑。`mean_wealth` 同时也是 §6 要求随
E1-C4 报告的财富尺度诊断量，值得纳入。
...
否则它就不是一次忠实的扩展，而是另一次测量。该检验可在本地对已入库的产物执行。
