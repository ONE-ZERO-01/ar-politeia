...
int main(int argc, char* argv[]) {
    ...
    if (cfg.terrain_type == "grid" && !cfg.terrain_file.empty()) {
        ...
        if (rank == 0) {
            std::cout << "Terrain: grid " << terrain_grid.ncols() << "x" << terrain_grid.nrows()
                      ...
                      << "] force=" << (cfg.terrain_force_enabled ? cfg.terrain_force_scale : 0.0)
                      << " production=" << (cfg.terrain_production_enabled ? cfg.terrain_production_scale : 0.0)
                      << "\n";
        }
    ...
    const politeia::TerrainGrid* force_grid_ptr =
        (cfg.terrain_force_enabled && terrain_grid.loaded()) ? &terrain_grid : nullptr;
    politeia::TerrainParams force_terrain = terrain;
    if (!cfg.terrain_force_enabled) {
        force_terrain.wells.clear();
    }
    politeia::LangevinIntegrator integrator(
        ...
        force_grid_ptr, cfg.terrain_force_scale
    );
    ...
    // --- Initial force computation (needed for first Verlet half-step) ---
    {
        ...
        if (cfg.terrain_force_enabled && terrain_grid.loaded()) {
            (void)politeia::compute_grid_terrain_forces(
                particles.x_data(), f.data(), particles.count(),
                terrain_grid, cfg.terrain_force_scale);
        } else if (cfg.terrain_force_enabled) {
        ...
        for (politeia::Index i = 0; i < particles.count(); ++i) {
            if (terrain_grid.loaded()) {
                terrain_at_particle[i] = politeia::grid_terrain_potential(
                    x[2*i], x[2*i+1], terrain_grid, 1.0);
            } else {
    ...
    for (politeia::Index step = start_step + 1; step <= cfg.total_steps; ++step) {
        ...
        if (cfg.climate_enabled && cfg.climate_production_enabled && climate_grid.loaded()) {
            ...
            for (politeia::Index i = 0; i < particles.count(); ++i) {
                ...
                if (cfg.terrain_production_enabled && terrain_at_particle.data() != nullptr) {
                    local_resource = cfg.base_production * cfg.terrain_production_scale
                                   * std::max(0.0, -terrain_at_particle[i]);
                }
        ...
        politeia::apply_resource_dynamics(
            ...
            cfg.terrain_production_enabled,
            cfg.terrain_production_scale,
        ...
        politeia::Index deaths = 0;
        if (cfg.mortality_enabled) {
            deaths = politeia::apply_mortality(particles, mort_params, cfg.dt, pop_rng);
        }
