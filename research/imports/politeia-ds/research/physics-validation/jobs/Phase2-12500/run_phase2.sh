#!/bin/bash
# Phase 2: China vs Europe × 12500步 完整对比
# 与 M1.16 continent_12500 形成三种地形矩阵
# 锁定 M1.16 STATE 配方: 10K×12500步×3 seeds×2 地形 = 6 runs

set -uo pipefail
BASE="/home/wanwb/ONE/Politeia-ds/research/physics-validation/jobs/Phase2-12500"
...
for terrain in china europe; do
  for seed in 42 123 456; do
    ...
    if [ $rc -eq 0 ]; then
      SUCES=$((SUCES + 1))
      echo "$(date '+%H:%M:%S') [$COUNT/6] $tag ✅"
      echo "  => $(tail -1 $BASE/output/${tag}/polity_summary.csv)"
    else
...
echo "=============================="
echo "$(date '+%H:%M:%S') Phase 2 DONE: $SUCES/$COUNT success"

echo ""
echo "=== 三种地形最终对比 ==="
echo "Continent (M1.16): $(tail -1 /home/wanwb/ONE/Politeia-ds/research/physics-validation/jobs/M1.13/output/f_1e-4_12500/polity_summary.csv)"
for terrain in china europe; do
  for seed in 42 123 456; do
    tag="${terrain}_s${seed}"
